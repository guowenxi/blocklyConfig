...
Toolbox.prototype.createContainer_ = function() {
  ...
  toolboxContainer.appendChild(scrollBox)
...
Toolbox.prototype.setSelectedItem = function(newItem) {
  ...
  scrollBox.style.top = `${newItem.rowDiv_.offsetTop}px`;
  setStyle(newItem.labelDom_,{
    transition:"all 0.3s",
    "transition-delay" : "0.1s"
  })
  let blocklyToolboxCategory = document.getElementsByClassName("blocklyToolboxCategory");
  for(var i =0 ; i<blocklyToolboxCategory.length; i++){
    blocklyToolboxCategory[i].getElementsByClassName("blocklyTreeLabel")[0].classList.remove("font-big")
  }
  newItem.labelDom_.classList.add("font-big");
...
Css.register(`
  ...
  .blocklyToolboxDiv {
    ...
    overflow-y: auto;
    position: absolute;
    box-shadow: -3px 0px 3px #f5f9fd, 3px 0px 8px #d8dbe5;
