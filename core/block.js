 ...
 * @alias Blockly.Block
 */
const Block = function(workspace, prototypeName, opt_id,state) {
 ...
 */
Block.prototype.getFieldName = function(name) {
...
Block.prototype.interpolate_ = function(
    message, args, lastDummyAlign, warningPrefix , state) {
  ...
  for (let i = 0, element; (element = elements[i]); i++) {
    if (this.isInputKeyword_(element['type'])) {
      ...
      const field = this.fieldFromJson_(element,this.state);
      if (field) {
 ...
 * @private
 */
Block.prototype.fieldFromJson_ = function(element,state) {
  const field = fieldRegistry.fromJson(element,state);
