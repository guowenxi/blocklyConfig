 ...
 */
const FieldDropdown = function(menuGenerator, opt_validator, opt_config,state) {
  ...
  if(state && state.fields){
    for(var key in state.fields){
      if(opt_config.name === key){
        selVal = state.fields[key];
      }
    }
  }
  // Call parent's constructor.
  FieldDropdown.superClass_.constructor.call(
      this, selVal || this.selectedOption_[1], opt_validator, opt_config,state);
  /**
 ...
 * @nocollapse
 */
FieldDropdown.fromJson = function(options,state) {
  // `this` might be a subclass of FieldDropdown if that class doesn't override
  // the static fromJson method.
  return new this(options['options'], undefined, options,state);
};
